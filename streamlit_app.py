...
st.set_page_config(
    page_title="Housing Price Predictor", page_icon="🏠", layout="centered"
)
...
with st.form("prediction_form"):
    ...
    with col1:
        MedInc = st.number_input(
            "Median Income (MedInc)", min_value=0.0, value=8.3252, format="%.4f"
        )
        HouseAge = st.number_input(
            "House Age", min_value=0.0, value=41.0, format="%.1f"
        )
        AveRooms = st.number_input(
            "Average Rooms", min_value=0.0, value=6.9841, format="%.4f"
        )
        AveBedrms = st.number_input(
            "Average Bedrooms", min_value=0.0, value=1.0238, format="%.4f"
        )
    with col2:
        Population = st.number_input(
            "Population", min_value=0.0, value=322.0, format="%.1f"
        )
        AveOccup = st.number_input(
            "Average Occupancy", min_value=0.0, value=2.5556, format="%.4f"
        )
